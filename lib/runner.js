...
function runOnce(claudeArgs, account, existingSessionId) {
  return new Promise((resolve) => {
    ...
    // For the default account (~/.claude) do NOT set CLAUDE_CONFIG_DIR.
    // Claude Code uses a hash-based keychain service name when the env var is
    // explicitly set — even if it points to ~/.claude — causing a mismatch
    // where credentials were stored under the standard name but looked up
    // under the hashed one, triggering an unwanted re-auth prompt.
    ...
    child.onData((data) => {
      ...
      if (match) {
        ...
        try { isWindows() ? child.kill() : child.kill('SIGTERM'); } catch {}
        setTimeout(() => {
